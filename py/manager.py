...
target_num_tokens_per_batch = 50000
...
def add_src_sequences(
    ...
    print('mgr: sending enc_input')
    mgr_to_enc_queue.put(enc_input)
    print('mgr: sent enc_input. waiting for encoded')
    encoded = enc_to_mgr_queue.get()
    print('manager: got encoded')
...
def run_manager(
    foo, input_path: str, output_path: str, mgr_to_enc_queue, enc_to_mgr_queue,
    mgr_to_dec_queue, dec_to_mgr_queue
):
    print(f"manager: foo {foo}")
    sequences: Dict[int, SequenceInfo] = {}
    spillover_sequences: Dict[int, SequenceInfo] = {}
    with open_buckets(input_path) as dataset, open(
        output_path + ".bin", "wb"
    ) as output_data_file, open(output_path + ".idx", "wb") as output_index_file, torch.no_grad():
        for bucket_idx in range(dataset.num_buckets):
            ...
            max_iterations = len(bucket_entries) * seq_len * 2  # Safety valve
            iteration_count = 0
            ...
            while True:
                iteration_count += 1
                print(f'manager: iteration {iteration_count}')
                if iteration_count > max_iterations:
                    print(f"Warning: Reached maximum iterations ({max_iterations}), breaking loop")
                    break
                # Try to get num_rows seqences we can process
                batch = dict(islice(sequences.items(), num_rows * 3))
                if not batch or len(batch) < num_rows:
                    num_seqs_added = add_src_sequences(
                        mgr_to_enc_queue, enc_to_mgr_queue, sequences, seq_len, bucket_iterator, num_rows * 6
                    )
                    if num_seqs_added == 0:
                        if not batch:
                ...
                mgr_to_dec_queue.put((enc_input_tensor, memory_tensor, dec_input_tensor))

                result = dec_to_mgr_queue.get()
        ...
        # We're done with all the buckets. If we still have spillover sequences, store empty results
        # for those
